...
function bayarIds(store: EventStore): string[] {
  return store.query<{ event_id: string }>(`SELECT event_id FROM bayar ORDER BY nominal`).map((r) => r.event_id);
}
...
describe('flfix-sync', () => {
  it('push: one un-storable event dead-letters instead of pinning the batch cursor', async () => {
    ...
    try {
      const e1 = log.append({ type: 'bayar', payload: { nominal: 1000, oleh: 'budi' } });
      const e2 = log.append({ type: 'bayar', payload: { nominal: 2000, oleh: 'budi' } });
      const e3 = log.append({ type: 'bayar', payload: { nominal: 3000, oleh: 'budi' } });
      ...
      assert.deepEqual(bayarIds(store), [e1.id, e3.id]);
  ...
  it('pull: one un-storable event dead-letters and the cursor still advances', async () => {
    ...
    try {
      const mk = (id: string, seq: number, nominal: number): LogEvent => ({
        id, seq, type: 'bayar', actor: 'budi', device_id: 'devA', ts_device: seq,
        payload: { nominal, oleh: 'budi' }, prev_hash: 'GENESIS', hash: `h-${id}`,
      });
      const relay = new MemoryRelay();
      ...
      assert.deepEqual(bayarIds(store), ['g-1', 'g-2']);
  ...
  it('failover re-push selects the acked prefix by seq bound, not seq arithmetic', async () => {
    ...
    try {
      ...
      const mk = (seq: number): LogEvent => ({
        id: `e${seq}`, seq, type: 'bayar', actor: 'budi', device_id: 'devA', ts_device: seq,
        payload: { nominal: 1000 + seq, oleh: 'budi' }, prev_hash: 'x', hash: `h${seq}`,
      });
  ...
  it('highValue threshold misconfig fails loud instead of dead-lettering payments', async () => {
    ...
    try {
      ...
      const mkRelayEv = (id: string, seq: number, extra?: Partial<LogEvent>): LogEvent => {
        const core = {
          id, seq, type: 'bayar', actor: 'budi', device_id: 'budi-dev', ts_device: seq,
          payload: { nominal: 1_000_000, oleh: 'budi' }, prev_hash: 'GENESIS',
        };
      ...
      assert.deepEqual(bayarIds(store), ['hv-2']);
