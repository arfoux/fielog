...
describe('deltasync audit fixes', () => {
  it('poison dead-letters by UUID: recorded once, never refetched', async () => {
    ...
    const peer = scriptedPeer(
      [
        mkRemote('good-1', 1),
        // Shape-invalid: checkAppend rejects a non-positive value.
        mkRemote('poison-1', 2, { payload: { value: -5, actor: 'budi' } }),
      ],
      seen,
    );
  ...
  it('origin-auth stripping: fresh local chain position, origin kept as metadata', async () => {
    ...
    assert.equal(ev.type, 'entry');
    assert.deepEqual(ev.payload, { value: 1000, actor: 'budi' });
  });
});
