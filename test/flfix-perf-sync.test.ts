...
function appendBayar(log: AppendLog, store: EventStore, nominal: number, deviceId?: string): LogEvent {
  const ev = log.append({ type: 'bayar', payload: { nominal, oleh: 'kasir' }, device_id: deviceId });
  store.apply(ev);
  return ev;
}

function relayBayar(id: string, seq: number, device: string, nominal: number): LogEvent {
  return {
...
describe('flfix-perf-sync', () => {
  it('purgeRevoked scans only the new suffix under stable revoke state', () => {
    ...
    try {
      for (let i = 0; i < 3; i++) appendBayar(log, store, 100 + i);
      ...
      for (let i = 0; i < 2; i++) appendBayar(log, store, 200 + i);
  ...
  it('purge cursor survives reopen via store meta', () => {
    ...
    for (let i = 0; i < 2; i++) appendBayar(log, store, 100 + i);
  ...
  it('grown revoke set falls back to a full rescan', () => {
    ...
    try {
      appendBayar(log, store, 100, 'devA');
      appendBayar(log, store, 200, 'devB');
      assert.deepEqual(purgeRevoked(log, store, { revokedDevices: ['devA'] }), { scanned: 2, quarantined: 1 });
      assert.deepEqual(
        store.query<{ event_id: string }>(`SELECT event_id FROM bayar`).map((r) => r.event_id).length,
        1,
      );
  ...
  it('pullRemote with revoke signal advances the persisted purge cursor', async () => {
    ...
    try {
      ...
      await relay.push([relayBayar('good-p', 1, 'devA', 1000), relayBayar('bad-p', 2, 'devB', 9000)]);
