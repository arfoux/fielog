...
function appendBayar(log: AppendLog, store: EventStore, i: number): void {
  const ev = log.append({
    type: 'bayar',
    payload: { nominal: 1000 + i, oleh: 'budi' },
    actor: 'budi',
    device_id: 'a',
  });
  store.apply(ev);
...
describe('deltasync manifest-first delta sync', () => {
  ...
  beforeEach(() => {
    ...
    for (let i = 0; i < N; i++) appendBayar(la, sa, i);
  });
  ...
  it('full sync: manifest first, want-list only, totals match', async () => {
    ...
    const rows = sb.query<{ total: number }>(`SELECT SUM(nominal) AS total FROM bayar WHERE voided = 0`);
    const want = sa.query<{ total: number }>(`SELECT SUM(nominal) AS total FROM bayar WHERE voided = 0`);
  ...
  it('mid-cut resume: persisted want-list completes with no duplicates', async () => {
    ...
    const partial = sb.query<{ n: number }>(`SELECT COUNT(*) AS n FROM bayar`)[0].n;
    ...
    assert.equal(sb.query<{ n: number }>(`SELECT COUNT(*) AS n FROM bayar`)[0].n, N);
    assert.equal(lb.readAll().length, N);
