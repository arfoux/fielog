...
describe('poison pull', () => {
  it('skips dead-letters, advances the cursor, keeps syncing', async () => {
    ...
    const good2: LogEvent = { id: 'good-2', seq: 4, type: 'entry', actor: 'budi', device_id: 'devA', ts_device: 4, payload: { value: 2000, actor: 'budi' }, prev_hash: 'h3', hash: 'h4' };
    ...
    try {
      ...
      const rows = await k.query<{ event_id: string; value: number }>(`SELECT event_id, value FROM entries ORDER BY value`);
      ...
      const good3: LogEvent = { id: 'good-3', seq: 5, type: 'entry', actor: 'budi', device_id: 'devA', ts_device: 5, payload: { value: 3000, actor: 'budi' }, prev_hash: 'h4', hash: 'h5' };
