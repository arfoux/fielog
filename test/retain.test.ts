...
describe('retention', () => {
  ...
  it('snapshot, truncate, and keep serving + syncing', async () => {
    ...
    let expected = 0;
    for (let i = 0; i < N; i++) {
      const value = 100 + (i % 997);
      expected += value;
      await k.append({ type: 'entry', value, actor: 'device' });
    }
    ...
    const snapRows = await k.query<{ total: number }>(`SELECT SUM(value) AS total FROM entries WHERE voided = 0`);
    ...
    for (let i = 0; i < 100; i++) {
      expected += 7;
      await k2.append({ type: 'entry', value: 7, actor: 'device' });
    }
    ...
    const rows3 = await k2.query<{ total: number }>(`SELECT SUM(value) AS total FROM entries WHERE voided = 0`);
