...
describe('stale-relay pull miss', () => {
  it('suffix living only on the second relay is applied', async () => {
    ...
    try {
      for (let i = 0; i < 5; i++) await seed.append({ type: 'bayar', nominal: 1000 + i, oleh: 'toko' });
    ...
    try {
      ...
      const rows = await reader.query<{ n: number }>(`SELECT COUNT(*) AS n FROM bayar WHERE voided = 0`);
      ...
      const rows2 = await reader.query<{ n: number }>(`SELECT COUNT(*) AS n FROM bayar WHERE voided = 0`);
  ...
  it('split replicas converge: stale prefix + fresh suffix both land', async () => {
    ...
    const seed = await createKernel({ file: join(dir, 'seed.db') });
    try {
      await seed.append({ type: 'bayar', nominal: 100, oleh: 'toko' });
      await seed.append({ type: 'bayar', nominal: 200, oleh: 'toko' });
      await seed.sync(stale, { ...fast });
      await seed.append({ type: 'bayar', nominal: 300, oleh: 'toko' });
    ...
    try {
      ...
      const rows = await reader.query<{ total: number }>(`SELECT SUM(nominal) AS total FROM bayar WHERE voided = 0`);
