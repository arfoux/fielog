...
describe('flfix-log: duplicate explicit ids', () => {
  it('append throws on a duplicate explicit id and writes nothing', () => {
    ...
    try {
      log.append({ type: 'note', payload: { content: 'a' }, id: 'dup-id' });
      assert.throws(
        () => log.append({ type: 'note', payload: { content: 'b' }, id: 'dup-id' }),
        /duplicate id/,
      );
      assert.equal(log.readAll().length, 1);
      assert.equal(log.getById('dup-id')?.payload['content'], 'a');
    } finally {
      log.close();
    }
  });
...
describe('flfix-log: verify seq continuity', () => {
  it('control: contiguous log still verifies clean', () => {
    ...
    try {
      log.append({ type: 'note', payload: { content: '1' } });
      log.append({ type: 'note', payload: { content: '2' } });
      log.append({ type: 'note', payload: { content: '3' } });
  ...
  it('flags a seq skip even when the survivor is re-chained (linkage intact)', () => {
    ...
    const log = openLog(path, 'devA');
    const e1 = log.append({ type: 'note', payload: { content: 'satu' } });
    log.append({ type: 'note', payload: { content: 'dua' } });
    const e3 = log.append({ type: 'note', payload: { content: 'tiga' } });
    log.close();
  ...
  it('flags a duplicated seq even when re-chained (linkage intact)', () => {
    ...
    const log = openLog(path, 'devA');
    const e1 = log.append({ type: 'note', payload: { content: 'satu' } });
    const e2 = log.append({ type: 'note', payload: { content: 'dua' } });
    log.close();
