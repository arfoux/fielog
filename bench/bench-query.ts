...
// workloads: full-table aggregate (SUM over bayar) and indexed point lookup by seq.
...
await measure('sum_all', () =>
  kernel.query(`SELECT SUM(nominal) AS total FROM bayar WHERE voided = 0`),
);
await measure('point_by_seq', (i) =>
  kernel.query(`SELECT * FROM bayar WHERE seq = ?`, [(i * 7919) % N + 1]),
);
kernel.close();
