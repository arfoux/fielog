...
const t1off = await node1.query<{ total: number }>(`SELECT SUM(amount) AS total FROM payment WHERE voided = 0`);
const t2off = await node2.query<{ total: number }>(`SELECT SUM(amount) AS total FROM payment WHERE voided = 0`);
...
console.log('match on both sides. device-01 state:', (await node1.query(`SELECT DISTINCT state FROM payment`)).map((r) => r.state));
