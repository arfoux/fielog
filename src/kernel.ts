...
export interface Kernel {
  ...
  verifyLog(): { ok: boolean; at?: number; reason?: string };
  close(): void;
}
...
function toAppendInput(args: AppendArgs, deviceId: string): AppendInput {
  ...
  void _i;
  void _t;
  return { type, actor, device_id: deviceId, payload: { ...clean, ...(payload ?? {}) } };
}
...
export async function createKernel(opts: KernelOpts): Promise<Kernel> {
  ...
  async function append(args: AppendArgs): Promise<LogEvent> {
    const input = toAppendInput(args, deviceId);
